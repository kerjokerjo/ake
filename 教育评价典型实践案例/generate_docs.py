...
def build_case_doc():
    ...
    body("以往工作主要依赖线下通知、人工汇总和经验判断，存在三方面突出问题：一是标准执行不统一，指标体系、评分规则、举证要求复杂，"
         "缺乏数字化承载，各校理解执行易产生偏差；二是审核评审效率低，传统系统仅校验材料数量、格式、大小，内容是否合规仍靠人工逐份查看，"
         "专家需阅读大量PDF和视频并撰写意见，单生评阅耗时长，且易意见趋同化、模板化；三是结果运用支撑弱，评审结果多以分数明细呈现，"
         "缺乏结构化诊断，学校难获改进信息，评价的诊断与反哺功能未能释放。基于此，项目团队提出“把政策要求转化为平台规则、把人工核查升级为智能预检、"
         "把专家评审升级为人机协同、把结果汇总升级为智能诊断”的思路，研发师范生教育教学能力认定抽检系统并创新嵌入三大AI智能体，"
         "以数字化与智能化双轮驱动，推动抽检从经验驱动走向规则驱动、从人工核查走向人机协同、从结果管理走向智能诊断。")
    figure("图1　系统总体架构与三智能体业务流程图")

    # ---------- 二、具体举措及实施成效 ----------
    h1("二、具体举措及实施成效")
    h2("（一）构建“三智能体”AI赋能体系，破解评价全链条共性难题")
    body("本案例最具突破性的创新，在于围绕抽检业务全链条精准嵌入三个差异化AI智能体，分别赋能学生与学校审核、专家评审、"
         "结果反馈三大关键环节，形成“贯通上下游、人机各司其职”的智能化质量监测体系。")

    h3("1.佐证材料“智能预检”，赋能学生上传与学校审核")
    body("在学生上传与校级审核环节嵌入智能预检智能体，构建“双场景预检”机制：学生提交前可一键启动预检，"
    ...
    body("在专家评审环节嵌入评审辅助智能体，提供“评审参考意见智能生成”服务：专家打开评审页面时，"
    ...
    body("构建认定结果分析智能体，在工具库评审基本完成后自动激活，基于全量数据生成涵盖总体概览、能力维度画像、指标达成分析、"
         "典型问题归纳、横向对比、改进建议等板块的“诊断书+处方”式报告。学校收到的不再是单一分数，"
    ...
    body("一是将抽检标准转化为可配置指标表，支持分层配置指标与评分规则，把文本标准变为可执行规则，为三个智能体提供核心规则依据；"
         "二是将评分办法转化为可计算规则，支持等级制与分数制，等级可设对应分值，兼顾专业判断与定量分析；"
         "三是将举证要求转化为材料清单，建立举证材料库（支持PDF、MP4，可设数量、大小、上传主体、排序），"
         "通过指标与材料多对多关联，明确“用什么材料证明什么能力”，为智能预检提供检测靶向。")

    h2("（三）推动流程闭环化，破解多方协同难题")
    body("一是构建多角色分权协同机制，设评估院管理员、学校管理员、学生、专家等角色，各司其职、边界清晰，AI在各环节差异化辅助；"
         "二是建立“学生上传—校级审核—评估院复核—专家评分”分级审核链条，“谁提交、谁审核、谁把关”清晰嵌入流程；"
         "三是实现在线查证与全过程留痕，专家在线查看关联佐证材料并评分，系统记录登录、操作、AI使用及流程节点日志，"
         "确保可查询、可追溯、可复盘。")

    h2("（四）推动规则公平化，统一抽样与评审")
    body("一是创新样本抽取规则，按专业人数动态设抽取比例，按同校同专业“按序等距抽取”，兼顾随机性、代表性与公平，"
         "抽取后可手动微调并确认名单；二是创新专家配置机制，支持一生多专家、同一专家按学校或指标范围承担任务，"
    ...
    body("平台采用“指标表+工具库+角色权限+流程配置+智能体”的模块化设计，三个智能体可基于不同项目的指标体系与材料类型快速适配，"
         ...
         "其“预检前置、辅助评审、智能诊断”理念还可迁移至专业认证、教育实习评价、课程达成度监测、学生发展评价等场景，具有较强推广价值。")

    h2("（六）实施成效")
    body("系统上线以来在效率、质量、覆盖、应用等维度成效显著。一是智能预检大幅提升审核效率与材料质量：单份材料审核时长由约20分钟降至约10分钟"
         "（降约50%），全市10余所高校累计完成500余份材料审核，500余名抽检师范生中超90%材料一次性通过校审、一次性通过率较改革前提升约30个百分点，"
    ...
    # ---------- 三、经验总结 ----------
    h1("三、经验总结")
    body("一是坚持AI辅助、人主决策。教育评价的专业性、敏感性、责任性决定AI不能替代人的判断，三个智能体始终坚守"
         "“AI识别问题、提供参考、辅助决策，人作最终判断、承担责任”的边界，既释放效率价值又守住专业底线。")
    body("二是坚持精准嵌入、靶向赋能。AI赋能不“撒胡椒面”，三个智能体分别针对审核效率、评审质量、结果运用三个环节的核心矛盾精准切入，"
         "避免“为AI而AI”的泛化应用。")
    body("三是坚持标准先行、数据贯通。智能体能力建立在结构化指标、规范化数据与闭环化流程之上，"
         "必须先把指标配置、举证管理、数据沉淀等基础打牢，智能化才能真正落地。")
    body("四是坚持闭环设计、持续改进。通过智能诊断把评价结果转化为改进建议，推动学校“评价—诊断—改进—再评价”良性循环，"
         "让教育评价真正发挥质量提升功能。")
    body("总体来看，本系统以“三智能体”AI赋能体系为核心创新，破解了审核效率低、评审负担重、结果运用浅等共性问题，"
